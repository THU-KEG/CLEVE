...
MODEL_CLASSES = {
    ...
    "roberta": (RobertaConfig, DMRoBERTa, RobertaTokenizer, RoBERTaContrastive),
}
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False,MI=False):
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
...
def main():
    ...
    parser.add_argument("--do_pretrain", action="store_true", help="Whether to do contrastive pre-training.")
    ...
    contrastive_model = contrastive_class.from_pretrained(
        args.model_name_or_path,
        from_tf=bool(".ckpt" in args.model_name_or_path),
        config=config,
        cache_dir=args.cache_dir if args.cache_dir else None,
    ...
    if args.do_pretrain:
        ...
        global_step, tr_loss ,best_steps = pretrain(args, pretrain_dataset, contrastive_model, tokenizer)
        logger.info(" contrast_global_step = %s, contrast_average loss = %s", global_step, tr_loss)
        return
