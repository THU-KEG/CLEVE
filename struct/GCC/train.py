...
def parse_option():
    ...
    #embedding
    parser.add_argument("--semantic-embedding-file", type=str, default="../glove.6B.300d.txt")
    parser.add_argument("--semantic-embedding-dim", type=int, default=300)
    parser.add_argument("--num-edge-categs", type=int, default=100)
    parser.add_argument("--edge-categ-embedding-size", type=int, default=32)
...
def load_embedding(glove_path, embedding_dim):
    ...
    pademb = np.zeros((1,embedding_dim), np.float32)
    wordemb = np.asarray(wordemb, np.float32)
    unkemb = np.mean(wordemb, axis=0, keepdims=True)
    return torch.tensor(np.concatenate([pademb, unkemb, wordemb], axis=0))
...
def main(args):
    ...
    wordemb = load_embedding(args.semantic_embedding_file, args.semantic_embedding_dim)
    ...
    model, model_ema = [
        GraphEncoder(
            ...
            num_edge_categs=args.num_edge_categs,
            edge_categ_embedding_size=args.edge_categ_embedding_size,
            wordemb=wordemb
        )
        for _ in range(2)
    ]
