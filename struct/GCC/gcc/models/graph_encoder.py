...
class GraphEncoder(nn.Module):
    ...
    def __init__(
        ...
        degree_input=False,
        lstm_as_gate=False,
        semantic_embedding_dim=300,
        num_edge_categs=100,
        edge_categ_embedding_size=32,
        wordemb=None
    ):
        super(GraphEncoder, self).__init__()

        if degree_input:
            node_input_dim = semantic_embedding_dim + positional_embedding_size + degree_embedding_size + 1
        else:
            node_input_dim = semantic_embedding_dim + positional_embedding_size + 1
        # node_input_dim = (
        #     positional_embedding_size + freq_embedding_size + degree_embedding_size + 3
        # )
        edge_input_dim = edge_categ_embedding_size
        if gnn_model == "mpnn":
        ...
        self.edge_categ_embedding = nn.Embedding(
            num_embeddings=num_edge_categs, embedding_dim=edge_categ_embedding_size
        )

        self.node_semantic_embedding = nn.Embedding.from_pretrained(wordemb, freeze=False, padding_idx=0)
    ...
    def forward(self, g, return_all_outputs=False):
        ...
        # nfreq = g.ndata["nfreq"]
        g_token_ids = g.ndata['token_ids']
        g_effective_tokens = torch.sum(g_token_ids != 0, dim=1, keepdim=True)
        g_token_emb = self.node_semantic_embedding(g_token_ids)
        g_token_emb = torch.sum(g_token_emb, dim=1) / g_effective_tokens
        ...
        e_feat = self.edge_categ_embedding(g.edata['type'])
        
        node_reps = None
        if self.gnn_model == "gin":
            x, all_outputs, node_reps = self.gnn(g, n_feat, e_feat)
        else:
